...
app.layout = html.Div([
    ...
    html.Div([
        ...
        style={'width': '48%', 'display': 'inline-block'}),
        dcc.Tabs(id="data_group", value='atmospara',children=[
            dcc.Tab(label='Atmospheric Parameters', value='atmospara'),
            dcc.Tab(label='Motion and Sound Detection', value='motionsound'),
        ]),
        html.Div(id="atmos_container",children=[
                html.Div([
        ...
        html.Div(id="motionsound_container",children=[
                html.Div([
                    dcc.Graph(id='pir'),
...
@app.callback(Output('atmos_container','style'),[Input('data_group','value')])
def render_graph(input):
    if input == 'atmospara':
        return {'display':'block'}
    else:
        return {'display':'none'}
@app.callback(Output('motionsound_container','style'),[Input('data_group','value')])
def render_graph(input):
    if input == 'motionsound':
        return {'display':'block'}
    else:
        return {'display':'none'}
# @app.callback(Output('tab_container','children'),[Input('data_group','value')])
...
@app.callback(Output('humidity', 'figure'), 
    ...
    Input('mean_rd', 'value'),
    Input('save_rd', 'value'),
    Input('data_group', 'value')])
def gen_humid_plot(interval, period, mean, save_plotting, tab):
    if tab == 'atmospara':
...
@app.callback(Output('temperature', 'figure'), 
    ...
    Input('save_rd', 'value'),
    Input('data_group', 'value')])
def gen_temp_plot(interval, period, mean, save_plotting, tab):
    if tab == 'atmospara':
        return live_plot(interval, period, mean, save_plotting, 'temperature')    
    else:
        return False

@app.callback(Output('pir', 'figure'), 
    [Input('update', 'n_intervals'),
    Input('period_rd', 'value'),
    Input('mean_rd', 'value'),
    Input('save_rd', 'value'),
    Input('data_group', 'value')])
def gen_pir_plot(interval, period, mean, save_plotting ,tab):
    if tab == 'motionsound':
        return live_plot(interval, period, mean, save_plotting, 'pir')
    else:
        return False
...
def gen_pir_plot(interval, period, mean, save_plotting ,tab):
    if tab == 'motionsound':
       return live_plot(interval, period, mean, save_plotting, 'soundDB')
    else:
        return False

@app.callback(Output('gas', 'figure'), 
    [Input('update', 'n_intervals'),
    Input('period_rd', 'value'),
    Input('mean_rd', 'value'),
    Input('save_rd', 'value'),
...
def gen_pir_plot(interval, period, mean, save_plotting,tab):
    if tab == 'atmospara':
        return live_plot(interval, period, mean, save_plotting, 'co')
    else:
        return False
